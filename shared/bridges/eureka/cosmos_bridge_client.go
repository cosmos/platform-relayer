...
import (
	...
	tendermint_v1_types "github.com/cosmos/eureka-relayer/proto/gen/ibc/lightclients/tendermint/v1"
	wasm_v1_types "github.com/cosmos/eureka-relayer/proto/gen/ibc/lightclients/wasm/v1"
...
func (client *CosmosBridgeClient) PacketWriteAckStatus(
	...
	if err != nil {
		return db.EurekaWriteAckStatusUNKNOWN, fmt.Errorf("hex decoding tx hash string %s: %w", hash, err)
	}
	...
	if err != nil {
		...
		return db.EurekaWriteAckStatusUNKNOWN, fmt.Errorf("getting tx %s's result: %w", hash, err)
	}
...
func parseWriteAckStatusForPacket(writeAckEvent abci.Event) (db.EurekaWriteAckStatus, error) {
	for _, attribute := range writeAckEvent.GetAttributes() {
		...
		if err != nil {
			return db.EurekaWriteAckStatusUNKNOWN, fmt.Errorf("%w:%w", ErrWriteAckDecoding, err)
		}
	...
	return db.EurekaWriteAckStatusUNKNOWN, nil
}
...
func (client *CosmosBridgeClient) SendPacketsFromTx(ctx context.Context, sourceChainID string, txHash string) ([]*PacketInfo, error) {
	...
	// We expect that every time we encounter an event with the dispatch_eureka_fee_bank_send attribute value
	// we will encounter a send packet event before encountering another dispatch_eureka_fee_bank_send occurrence.
	// So we populate curPayment every time we encounter a dispatch_eureka_fee_bank_send occurrence with the payment info
	// and in the next send packet event we process, we set the PacketInfo.Payment with curPayment and then set curPayment to nil.
	// We may encounter a send packet event without encountering a eureka payment in the events first. In this case curPayment
	// should be null, and PacketInfo.Payment will be null for that packet.
	...
	for _, event := range txResult.TxResult.GetEvents() {
		if event.GetType() == eventTypeSendPacket {
			...
			// associate last seen payment with the packet
			if curPayment != nil {
				packet.Payment = curPayment
				curPayment = nil
			}
			packets = append(packets, &packet)
		} else if event.GetType() == eventTypeWasm {
			var payment Payment
			var containsEurekaFeeBankSend bool
			for _, attribute := range event.GetAttributes() {
				switch attribute.GetKey() {
				case "action":
					if attribute.GetValue() == "dispatch_eureka_fee_bank_send" {
						containsEurekaFeeBankSend = true
					}
				case "eureka_fee_denom":
					payment.Denom = attribute.GetValue()
				case "eureka_fee_amount":
					payment.Amount = attribute.GetValue()
				case "eureka_fee_receiver":
					payment.Recipient = attribute.GetValue()
				}
			}
			if containsEurekaFeeBankSend {
				if curPayment != nil {
					return nil, fmt.Errorf("encountered multiple smart relay payment events before send packet event")
				}
				curPayment = &payment
			}
		}
