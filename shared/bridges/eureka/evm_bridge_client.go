...
import (
	...
	"github.com/cosmos/eureka-relayer/shared/contracts/erc20"
	"github.com/cosmos/eureka-relayer/shared/contracts/eureka_handler"
	"github.com/cosmos/eureka-relayer/shared/contracts/ics20_transfer"
	"github.com/cosmos/eureka-relayer/shared/contracts/ics26_router"
	"github.com/cosmos/eureka-relayer/shared/contracts/sp1_ics07_tendermint"
	"github.com/cosmos/eureka-relayer/shared/lmt"
	"github.com/cosmos/eureka-relayer/shared/signing"
	signingevm "github.com/cosmos/eureka-relayer/shared/signing/evm"
...
type EVMBridgeClient struct {
	...
	eurekaHandler *eureka_handler.EurekaHandler
...
func NewEVMBridgeClient(
	...
	eurekaHandlerContractAddress string,
	client EVMClient,
	signer signing.Signer,
	gasFeeCapMultiplier *float64,
	gasTipCapMultiplier *float64,
	...
	eurekaHandlerAddress := common.HexToAddress(eurekaHandlerContractAddress)
	eurekaHandler, err := eureka_handler.NewEurekaHandler(eurekaHandlerAddress, client)
	if err != nil {
		return nil, fmt.Errorf("creating contract binding for ics26 router at %s: %w", routerAddress, err)
	}

	// Validate signer address is exactly 20 bytes (Ethereum address length)
...
func (client *EVMBridgeClient) PacketWriteAckStatus(
	...
	sourceClientID string,
	destClientID string,
) (db.EurekaWriteAckStatus, error) {
	abi, err := ics26_router.ContractMetaData.GetAbi()
	if err != nil {
		return db.EurekaWriteAckStatusUNKNOWN, fmt.Errorf("getting ics26 router abi: %w", err)
	}
	...
	if err != nil {
		if strings.Contains(err.Error(), "not found") {
			return db.EurekaWriteAckStatusUNKNOWN, ErrTxNotFound
		}
		return db.EurekaWriteAckStatusUNKNOWN, fmt.Errorf("getting transaction receipt: %w", err)
	}
	...
	for _, log := range receipt.Logs {
		...
		if err != nil {
			return db.EurekaWriteAckStatusUNKNOWN, fmt.Errorf("parsing write packet event from log %v: %w", *log, err)
		}
		...
		if writeAck.Packet.Sequence == sequence && writeAck.Packet.SourceClient == sourceClientID && writeAck.Packet.DestClient == destClientID {
			if len(writeAck.Acknowledgements) == 1 {
				if bytes.Equal(writeAck.Acknowledgements[0], ErrorAcknowledgement[:]) {
					return db.EurekaWriteAckStatusERROR, nil
				}
			}
	...
	return db.EurekaWriteAckStatusUNKNOWN, ErrWriteAckNotFoundForPacket
}
...
func (client *EVMBridgeClient) SendPacketsFromTx(ctx context.Context, sourceChainID string, txHash string) ([]*PacketInfo, error) {
	...
	eurekaHandlerABI, err := eureka_handler.EurekaHandlerMetaData.GetAbi()
	if err != nil {
		return nil, fmt.Errorf("getting eureka handler abi: %w", err)
	}
	...
	var curPacket *PacketInfo
	const sendPacketEvent = "SendPacket"
	const eurekaTransferEvent = "EurekaTransfer"
	for _, log := range receipt.Logs {
		if log == nil {
			continue
		}
		...
		} else if eurekaHandlerABI.Events[eurekaTransferEvent].ID == log.Topics[0] {
			transfer, err := client.eurekaHandler.ParseEurekaTransfer(*log)
			if err != nil {
				return nil, fmt.Errorf("parsing transfer event from tx %s logs on chain %s: %w", txHash, client.chainID, err)
			}
			if transfer.RelayFee == nil {
				return nil, fmt.Errorf("amount in parsed transfer event is nil for tx %s on chain %s", txHash, client.chainID)
			}
			if curPacket == nil {
				return nil, fmt.Errorf("encountered payment without first encountering transfer for tx %s on chain %s", txHash, client.chainID)
			}
			curPacket.Payment = &Payment{
				Denom:     transfer.Token.Hex(),
				Amount:    transfer.RelayFee.String(),
				Recipient: transfer.RelayFeeRecipient.Hex(),
			}
			packets = append(packets, curPacket)
			curPacket = nil
		}
