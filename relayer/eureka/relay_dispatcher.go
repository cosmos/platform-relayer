...
type UnfinishedEurekaTransferStorage interface {
	// GetUnfinishedEurekaTransfers gets all eureka transfers that are not
	// marked as completed or failed in the db. Note that any of these
	// transfers may be in the current pipeline.
	GetUnfinishedEurekaTransfers(ctx context.Context) ([]db.EurekaTransfer, error)

	UpdateTransferState(ctx context.Context, arg db.UpdateTransferStateParams) error
}

type PipelineManager interface {
	Pipeline(ctx context.Context, transfer *EurekaTransfer) (EurekaPipeline, error)
	Close()
}

type RelayDispatcher struct {
	storage         UnfinishedEurekaTransferStorage
...
func (r *RelayDispatcher) Run(ctx context.Context) error {
	...
	// check if eureka relaying is enabled or not
	if !r.defaultEnabled {
		lmt.Logger(ctx).Info("eureka relaying disabled", zap.Bool("default_enabled", r.defaultEnabled))
		ticker.Stop()
	} else {
		lmt.Logger(ctx).Info("eureka relaying enabled", zap.Bool("default_enabled", r.defaultEnabled))
	}
...
func (r *RelayDispatcher) SubmitWaitingUnfinishedTransfers(ctx context.Context) error {
	transfers, err := r.storage.GetUnfinishedEurekaTransfers(ctx)
	if err != nil {
		return fmt.Errorf("getting unfinished eureka transfers from storage: %w", err)
	}

	for _, transfer := range transfers {
		t := NewEurekaTransfer(ctx, transfer)
		...
		if err = r.SubmitTransfer(ctx, t); err != nil && !errors.Is(err, ErrTransferAlreadyInPipeline) {
			...
			update := db.UpdateTransferStateParams{
				Status:               db.EurekaRelayStatusFAILED,
...
// SubmitTransfer submits a transfer to be added to the eureka packet
...
func (r *RelayDispatcher) SubmitTransfer(ctx context.Context, transfer *EurekaTransfer) error {
