...
func (processor RecvTxGasCalculatorProcessor) Process(ctx context.Context, transfer *EurekaTransfer) (*EurekaTransfer, error) {
	destinationBridgeClient, err := processor.bridgeClientManager.GetClient(ctx, transfer.GetDestinationChainID())
	if err != nil {
		return nil, fmt.Errorf("getting eureka bridge client for transfer destination chain %s: %w", transfer.GetDestinationChainID(), err)
	}
...
func (processor RecvTxGasCalculatorProcessor) Cancel(transfer *EurekaTransfer, err error) {
...
func (processor RecvTxGasCalculatorProcessor) ShouldProcess(transfer *EurekaTransfer) bool {
...
func (processor RecvTxGasCalculatorProcessor) State() db.EurekaRelayStatus {
	return db.EurekaRelayStatusCALCULATINGRECVTXGASCOST
}
