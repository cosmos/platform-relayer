...
func (processor WaitForWriteAckPacket) Process(ctx context.Context, transfer *EurekaTransfer) (*EurekaTransfer, error) {
	...
	if errors.Is(err, eureka.ErrWriteAckNotFoundForPacket) {
	...
	if err != nil && !errors.Is(err, eureka.ErrWriteAckDecoding) {
	...
	if errors.Is(err, eureka.ErrWriteAckDecoding) {
		...
		status = db.EurekaWriteAckStatusUNKNOWN
	}
	...
	update := db.UpdateTransferWriteAckTxParams{
		...
		WriteAckStatus:       db.NullEurekaWriteAckStatus{Valid: true, EurekaWriteAckStatus: status},
		SourceChainID:        transfer.GetSourceChainID(),
		PacketSourceClientID: transfer.GetPacketSourceClientID(),
		PacketSequenceNumber: int32(transfer.GetPacketSequenceNumber()),
	}
...
func (processor WaitForWriteAckPacket) Cancel(transfer *EurekaTransfer, err error) {
...
func (processor WaitForWriteAckPacket) ShouldProcess(transfer *EurekaTransfer) bool {
...
func (processor WaitForWriteAckPacket) State() db.EurekaRelayStatus {
	return db.EurekaRelayStatusWAITFORWRITEACK
}
