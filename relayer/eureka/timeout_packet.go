...
func (processor BatchTimeoutPacketProcessor) Process(ctx context.Context, transfers []*EurekaTransfer) ([]*EurekaTransfer, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("getting timeout tx bytes from eureka relayer for batch of %d transfers: %w", len(sequences), err)
	}
...
func (processor BatchTimeoutPacketProcessor) Cancel(transfers []*EurekaTransfer, err error) {
...
func (processor BatchTimeoutPacketProcessor) ShouldProcess(transfer *EurekaTransfer) bool {
...
func (processor BatchTimeoutPacketProcessor) State() db.EurekaRelayStatus {
