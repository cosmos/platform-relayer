...
import (
	...
	mock_eureka_bridge "github.com/cosmos/eureka-relayer/mocks/shared/bridges/eureka"
...
func TestPipeline(t *testing.T) {
	t.Run("single transfer through pipeline, no errors", func(t *testing.T) {
		...
		mockStorage := mock_eureka.NewMockStorage(t)
		...
		clients := map[string]eureka_bridge.BridgeClient{sourceChainID: mockSourceBridgeClient, destChainID: mockDestBridgeClient}
		bridgeClientManager := eureka.NewClientManager(clients)
		...
		recvRequest := &eurekarelayer.RelayByTxRequest{
		...
		recvResponse := &eurekarelayer.RelayByTxResponse{Tx: recvTxBytes}
		...
		assert.NoError(t, err)
		ackRequest := &eurekarelayer.RelayByTxRequest{
		...
		ackResponse := &eurekarelayer.RelayByTxResponse{Tx: ackTxBytes}
		mockRelayService.EXPECT().RelayByTx(mock.Anything, ackRequest).Return(ackResponse, nil).Once()

		priceClient := mock_eureka.NewMockPriceClient(t)

		opts := eureka.NewSmallPipelineOpts()
		opts.ShouldRelaySuccessAcks = true
		pipeline := eureka.NewPipeline(
			ctx,
			mockStorage,
			bridgeClientManager,
			mockRelayService,
		...
		pipeline.Input <- &eureka.EurekaTransfer{
		...
		assert.Equal(t, db.EurekaRelayStatusCOMPLETEWITHACK, output.GetState())
	...
	t.Run("recv tx not found for two iterations, then is found", func(t *testing.T) {
		...
		mockSourceBridgeClient := &MockAcceptingBridgeClient{}
		mockDestBridgeClient := mock_eureka_bridge.NewMockBridgeClient(t)
		clients := map[string]eureka_bridge.BridgeClient{sourceChainID: mockSourceBridgeClient, destChainID: mockDestBridgeClient}
		bridgeClientManager := eureka.NewClientManager(clients)
		...
		mockDestBridgeClient.EXPECT().DeliverTx(mock.Anything, recvResponse.GetTx(), recvResponse.GetAddress()).Return(&eureka_bridge.BridgeTx{Hash: mockTxHash, Timestamp: mockTxTime}, nil).Once()
		...
		// has then been found
		mockDestBridgeClient.EXPECT().ShouldRetryTx(mock.Anything, mockTxHash, eureka.RetryRecvExpiry, mockTxTime).Return(false, eureka_bridge.ErrTxNotFound).Twice()
		mockDestBridgeClient.EXPECT().ShouldRetryTx(mock.Anything, mockTxHash, eureka.RetryRecvExpiry, mockTxTime).Return(false, nil).Once()

		mockDestBridgeClient.EXPECT().IsTxFinalized(mock.Anything, mockTxHash, mock.Anything).Return(true, nil)
		mockDestBridgeClient.EXPECT().PacketWriteAckStatus(mock.Anything, mockTxHash, uint64(packetSequenceNumber), sourceClientID, destClientID).Return(db.EurekaWriteAckStatusSUCCESS, nil)
		...
		assert.True(t, pipeline.Push(ctx, &eureka.EurekaTransfer{
			State:                     db.EurekaRelayStatusPENDING,
			SourceChainID:             sourceChainID,
			DestinationChainID:        destChainID,
			SourceTxHash:              mockTxHash,
		...
		assert.NoError(t, err)
		assert.Equal(t, db.EurekaRelayStatusDELIVERRECVPACKET, output.GetState())
		assert.Equal(t, mockTxHash, *output.RecvTxHash)
		assert.Equal(t, mockTxTime, *output.RecvTxTime)
	...
	t.Run("recv tx not found, then times out and is retried", func(t *testing.T) {
		...
		mockDestBridgeClient.EXPECT().ShouldRetryTx(mock.Anything, mockTxHash, eureka.RetryRecvExpiry, mockTxTime).Return(false, eureka_bridge.ErrTxNotFound).Once()
		mockDestBridgeClient.EXPECT().ShouldRetryTx(mock.Anything, mockTxHash, eureka.RetryRecvExpiry, mockTxTime).Return(true, nil).Once()
	...
	t.Run("success ack transfer, pipeline does not relay success acks", func(t *testing.T) {
		...
		mockDestBridgeClient.EXPECT().DeliverTx(mock.Anything, recvResponse.GetTx(), recvResponse.GetAddress()).Return(&eureka_bridge.BridgeTx{Hash: mockTxHash, Timestamp: mockTxTime}, nil)
		mockDestBridgeClient.EXPECT().ChainType().Return(config.ChainType_COSMOS)
		mockDestBridgeClient.EXPECT().ShouldRetryTx(mock.Anything, mockTxHash, eureka.RetryRecvExpiry, mockTxTime).Return(false, nil)
		mockDestBridgeClient.EXPECT().PacketWriteAckStatus(mock.Anything, mockTxHash, uint64(packetSequenceNumber), sourceClientID, destClientID).Return(db.EurekaWriteAckStatusSUCCESS, nil)
		...
		pipeline.Push(ctx, &eureka.EurekaTransfer{
			State:                     db.EurekaRelayStatusPENDING,
			SourceChainID:             sourceChainID,
			DestinationChainID:        destChainID,
			SourceTxHash:              mockTxHash,
		...
		output, err := pipeline.Poll()
		assert.NoError(t, err)
		assert.Equal(t, db.EurekaRelayStatusCOMPLETEWITHWRITEACKSUCCESS, output.GetState())
		assert.Equal(t, mockTxHash, *output.RecvTxHash)
		assert.Equal(t, mockTxTime, *output.RecvTxTime)
		assert.Equal(t, mockTxHash, *output.WriteAckTxHash)
		assert.Equal(t, mockTxTime, *output.WriteAckTxTime)
		assert.Equal(t, db.EurekaWriteAckStatusSUCCESS, *output.WriteAckStatus)
		assert.Nil(t, output.AckTxHash)
		assert.Nil(t, output.AckTxTime)
	})
	...
	t.Run("timed out transfer cosmos destination chain", func(t *testing.T) {
		...
		timeoutResponse := &eurekarelayer.RelayByTxResponse{Tx: timeoutTxBytes}
		...
		output := <-pipeline.Output
		assert.Equal(t, db.EurekaRelayStatusCOMPLETEWITHTIMEOUT, output.GetState())
	...
	t.Run("timed out transfer evm destination chain", func(t *testing.T) {
		...
		output := <-pipeline.Output
		assert.Equal(t, db.EurekaRelayStatusAWAITINGTIMEOUTFINALITY, output.GetState())
	...
	t.Run("error ack transfer, pipeline does not relay error acks", func(t *testing.T) {
		...
		mockDestBridgeClient.EXPECT().ChainType().Return(config.ChainType_COSMOS)
		mockDestBridgeClient.EXPECT().ShouldRetryTx(mock.Anything, mockTxHash, eureka.RetryRecvExpiry, mockTxTime).Return(false, nil)
		mockDestBridgeClient.EXPECT().PacketWriteAckStatus(mock.Anything, mockTxHash, uint64(packetSequenceNumber), sourceClientID, destClientID).Return(db.EurekaWriteAckStatusERROR, nil)
		...
		output, err := pipeline.Poll()
		assert.NoError(t, err)
		assert.Equal(t, db.EurekaRelayStatusCOMPLETEWITHWRITEACKERROR, output.GetState())
		assert.Equal(t, mockTxHash, *output.RecvTxHash)
		assert.Equal(t, mockTxTime, *output.RecvTxTime)
		assert.Equal(t, mockTxHash, *output.WriteAckTxHash)
		assert.Equal(t, mockTxTime, *output.WriteAckTxTime)
		assert.Equal(t, db.EurekaWriteAckStatusERROR, *output.WriteAckStatus)
		assert.Nil(t, output.AckTxHash)
		assert.Nil(t, output.AckTxTime)
	})
	...
	t.Run("unknown ack status transfer, pipeline does not relay error acks", func(t *testing.T) {
		...
		mockDestBridgeClient.EXPECT().ChainType().Return(config.ChainType_COSMOS)
		mockDestBridgeClient.EXPECT().ShouldRetryTx(mock.Anything, mockTxHash, eureka.RetryRecvExpiry, mockTxTime).Return(false, nil)
		mockDestBridgeClient.EXPECT().PacketWriteAckStatus(mock.Anything, mockTxHash, uint64(packetSequenceNumber), sourceClientID, destClientID).Return(db.EurekaWriteAckStatusUNKNOWN, nil)
		...
		assert.Equal(t, mockTxHash, *output.WriteAckTxHash)
		assert.Equal(t, mockTxTime, *output.WriteAckTxTime)
		assert.Equal(t, db.EurekaWriteAckStatusUNKNOWN, *output.WriteAckStatus)
		assert.Nil(t, output.AckTxHash)
		assert.Nil(t, output.AckTxTime)
	})
}
...
func TestPipelineDeduper(t *testing.T) {
	t.Run("cant push duplicated transfers onto the pipeline", func(t *testing.T) {
		...
		deduper := eureka.NewPipelineDeduper(mockPipeline)
...
func (mock *MockAcceptingBridgeClient) FindRecvTx(ctx context.Context, sourceClientID string, destClientID string, sequence uint64, timeoutTimestamp time.Time) (*eureka_bridge.BridgeTx, error) {
	panic("unimplemented")
}

func (mock *MockAcceptingBridgeClient) FindAckTx(ctx context.Context, sourceClientID string, destClientID string, sequence uint64) (*eureka_bridge.BridgeTx, error) {
	panic("unimplemented")
}

func (mock *MockAcceptingBridgeClient) FindTimeoutTx(ctx context.Context, sourceClientID string, destClientID string, sequence uint64) (*eureka_bridge.BridgeTx, error) {
	panic("unimplemented")
}

func (mock *MockAcceptingBridgeClient) DeliverTx(ctx context.Context, tx []byte, address string) (*eureka_bridge.BridgeTx, error) {
	return &eureka_bridge.BridgeTx{Hash: mockTxHash, Timestamp: mockTxTime}, nil
}

func (mock *MockAcceptingBridgeClient) PacketWriteAckStatus(ctx context.Context, hash string, sequence uint64, sourceClientID string, destClientID string) (db.EurekaWriteAckStatus, error) {
	return db.EurekaWriteAckStatusSUCCESS, nil
}

func (mock *MockAcceptingBridgeClient) SendPacketsFromTx(ctx context.Context, sourceChainID string, txHash string) ([]*eureka_bridge.PacketInfo, error) {
	return nil, nil
}
...
func (mock *MockAcceptingBridgeClient) ClientState(ctx context.Context, clientID string) (eureka_bridge.ClientState, error) {
	return eureka_bridge.ClientState{}, nil
}
