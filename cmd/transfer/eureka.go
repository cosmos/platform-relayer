...
import (
	...
	"github.com/cosmos/eureka-relayer/proto/gen/relayerapi"
...
func eurekaTransfer(ctx context.Context) error {
	flag.Parse()
	...
	if !slices.Contains(sourceChain.SupportedBridges, config.BridgeType_EUREKA) {
		return fmt.Errorf("source chain does not support eureka")
	}
	...
	if !slices.Contains(destChain.SupportedBridges, config.BridgeType_EUREKA) {
		return fmt.Errorf("dest chain does not support eureka")
	}
	...
	sourceChainEurekaConfig, err := config.GetConfigReader(ctx).GetEurekaConfig(*sourceChainID)
	if err != nil {
		return fmt.Errorf("loading source chain eureka config: %w", err)
	}
	clientCounterpartyChainID, ok := sourceChainEurekaConfig.CounterpartyChains[*sourceClientID]
	if !ok {
		return fmt.Errorf("could not determine counterparty chain id for source client %s (mapping for counterparty client is likely missing for chains eureka config)", *sourceClientID)
	}
	...
	clientManager, err := eureka.NewClientManagerFromConfig(ctx, map[string]string{*sourceChainID: *privateKey}, nil, "", "", sourceChain)
	if err != nil {
		return fmt.Errorf("creating eureka client manager: %w", err)
	}

	sourceClient, err := clientManager.GetClient(ctx, *sourceChainID)
	if err != nil {
		return fmt.Errorf("getting eureka client for source chain id %s: %w", *sourceChainID, err)
	}

	lmt.Logger(ctx).Info(
		fmt.Sprintf("sending eureka transfer from %s to %s", *sourceChainID, *destChainID),
	...
	if err != nil {
		return fmt.Errorf("sending eureka transfer from %s to %s: %w", *sourceChainID, *destChainID, err)
	}

	lmt.Logger(ctx).Info(
		fmt.Sprintf("successfully sent eureka transfer from %s to %s", *sourceChainID, *destChainID),
